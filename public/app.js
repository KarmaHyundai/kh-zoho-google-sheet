...
async function api(action,data={}){
  const response=await fetch(API_URL,{
    ...
    body:JSON.stringify({action,sessionToken:state.sessionToken,...data})
  });
...
function formatRole(role){
  ...
  if(r==='VERTICAL_HEAD')return'Vertical Head';
...
function handleSessionError(err){
  ...
  if(text.includes('session has expired')||text.includes('access is no longer active')){
    clearLocalSession();
...
async function sendOtp(){
  ...
  try{
    ...
    setMessage('loginMessage',out.message,'success');
    $('otp').focus();
  }catch(err){
...
function renderVerticals(){
  ...
  select.innerHTML=
    ...
    state.verticals.map(v=>
      '<option value="'+escapeHtml(v.vertical)+'">'+escapeHtml(v.vertical)+'</option>'
    ).join('');

  if(current&&state.verticals.some(v=>v.vertical===current)){
...
function updateApproverHint(){
  ...
  if(vertical.approverName1||vertical.approverEmail1){
    approvers.push(vertical.approverName1||vertical.approverEmail1);
  }
  if(vertical.approverName2||vertical.approverEmail2){
    approvers.push(vertical.approverName2||vertical.approverEmail2);
  }

  hint.textContent=approvers.length>1
    ? 'Approval will go to: '+approvers.join(' or ')+'. The first decision completes the request.'
    : 'Approval will go to: '+(approvers[0]||'Configured Vertical Head');
...
async function submitRequest(){
  ...
  if(!sheetName||!purpose||!vertical){
    setMessage('requestMessage','Google Sheet name, purpose and vertical are required.','error');
    return;
  }

  busy(true,'Submitting request…');
  try{
    const out=await api('submitRequest',{sheetName,purpose,vertical});
    ...
    setMessage(
      'requestMessage',
      out.requestId+' submitted successfully. Approver: '+out.approver,
      'success'
    );
...
async function loadMyRequests(){
  ...
  if(!out.requests.length){
    body.innerHTML='<tr><td colspan="5" class="muted">No requests submitted yet.</td></tr>';
    return;
  }

  body.innerHTML=out.requests.map(r=>{
    const note=r.adminNote
      ? '<br><span class="muted">'+escapeHtml(r.adminNote)+'</span>'
      : '';

    const action=r.sheetUrl
      ? '<a class="link" target="_blank" rel="noopener" href="'+escapeHtml(r.sheetUrl)+'">Open Sheet</a>'
      : '-';
...
async function loadApprovalQueue(){
  ...
  if(!out.requests.length){
    body.innerHTML='<tr><td colspan="7" class="muted">No requests in this view.</td></tr>';
    return;
  }

  body.innerHTML=out.requests.map(r=>{
    const status=String(r.status||'').toUpperCase();
    const canApproveNow=['PENDING','ERROR','PROVISIONING'].includes(status);
    const canRejectNow=!['APPROVED','REJECTED'].includes(status);

    let actions='<div class="actions">';

    if(r.sheetUrl){
      actions+='<a class="btn secondary small" target="_blank" rel="noopener" href="'+
        escapeHtml(r.sheetUrl)+'">Open</a>';
    }
    ...
    const decision=[
      r.actionBy?'By: '+escapeHtml(r.actionBy):'',
      r.adminNote?escapeHtml(r.adminNote):''
...
async function rejectRequest(requestId){
  const reason=prompt('Reason for rejecting '+requestId+':');
  if(!reason||!reason.trim())return;

  busy(true,'Rejecting request…');
  try{
    const out=await api('rejectRequest',{requestId,reason:reason.trim()});
    alert(out.message);
    await refreshApplication();
  }catch(err){
    handleSessionError(err);
